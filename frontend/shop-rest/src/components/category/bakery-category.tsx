      ...
      {data?.categories?.data?.length ? (
        <div className="pt-5 px-4 lg:p-8 lg:pb-0">
          <div className="relative">
              ...
              {data?.categories?.data.map((category, idx) => (
                <SwiperSlide key={idx}>
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold text-accent text-center px-4 pb-4 block">
                      {category.name}
                    </span>
                  </div>
                </SwiperSlide>
            ...
            <button className="banner-slider-prev w-8 h-8 flex items-center justify-center text-accent bg-light shadow-300 outline-none rounded-full absolute top-1/2 -mt-4 z-10 cursor-pointer -start-4 focus:outline-none">
              <span className="sr-only">{t("text-previous")}</span>
              <ArrowPrevIcon />
            </button>
            <button className="banner-slider-next w-8 h-8 flex items-center justify-center text-accent bg-light shadow-300 outline-none rounded-full absolute top-1/2 -mt-4 z-10 cursor-pointer -end-4 focus:outline-none">
